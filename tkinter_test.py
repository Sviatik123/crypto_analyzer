...
def generate_request(columns, db_table):
    ...
    if order_combobox.get() in form_column_list():
        request += " ORDER BY " + order_combobox.get()
    return request
...
def fill_table():
    ...
    for col in required_columns:
        table_treeview.column(col, stretch=False, width=68)
...
label_currencies = tk.Label(instrument, text='Select currency: ')
label_order = tk.Label(instrument, text='Order by: ')
...
check_btn6 = tk.Checkbutton(instrument, text='daily_change_relative', variable=check6, onvalue='daily_change_relative',
                            offvalue='')
...
check_btn14 = tk.Checkbutton(instrument, text='frr_amount_available', variable=check14, onvalue='frr_amount_available',
                             offvalue='')

columns = ['symbol', 'bid', 'bid_size', 'ask', 'ask_size', 'daily_change', 'daily_change_relative', 'last_price',
           'volume', 'high', 'low', 'frr', 'bid_period', 'ask_period', 'frr_amount_available']

order_combobox = ttk.Combobox(instrument, values=columns)
order_combobox.current(0)

check_btn1.grid(row=3, column=0, sticky=tk.W)
check_btn2.grid(row=3, column=1, sticky=tk.W, padx=(0, 10))
check_btn3.grid(row=4, column=0, sticky=tk.W)
check_btn4.grid(row=4, column=1, sticky=tk.W, padx=(0, 10))
check_btn5.grid(row=5, column=0, sticky=tk.W)
check_btn6.grid(row=5, column=1, sticky=tk.W, padx=(0, 10))
check_btn7.grid(row=6, column=0, sticky=tk.W)
check_btn8.grid(row=6, column=1, sticky=tk.W, padx=(0, 10))
check_btn9.grid(row=7, column=0, sticky=tk.W)
check_btn10.grid(row=7, column=1, sticky=tk.W, padx=(0, 10))
check_btn11.grid(row=8, column=0, sticky=tk.W)
check_btn12.grid(row=8, column=1, sticky=tk.W, padx=(0, 10))
check_btn13.grid(row=9, column=0, sticky=tk.W)
check_btn14.grid(row=9, column=1, sticky=tk.W, padx=(0, 10))

radio = tk.StringVar()

radio_btn1 = tk.Radiobutton(instrument, text='Pair', variable=radio, value='tickers', command=change_checkboxes)
radio_btn2 = tk.Radiobutton(instrument, text='Currency', variable=radio, value='tickers_on_currency',
                            command=change_checkboxes)
radio_btn1.select()
radio_btn2.deselect()
radio_btn1.grid(row=11, column=0, sticky=tk.W)
radio_btn2.grid(row=11, column=1, sticky=tk.W)
...
label_currencies.grid(row=0, column=0, padx=(0, 0), pady=(10, 10))
currencies_combobox.grid(row=0, column=1, padx=(0, 10), pady=(10, 10))
label_order.grid(row=1, column=0, padx=(0, 0), pady=(10, 10), sticky=tk.W)
order_combobox.grid(row=1, column=1, padx=(0, 10), pady=(10, 10))
form_btn.grid(row=12, column=1, sticky=tk.E, padx=(5, 10), pady=(10, 0))
label_checkboxes.grid(row=2, column=0, sticky=tk.W, pady=(10, 0))
label_radio.grid(row=10, column=0, sticky=tk.W, pady=(10, 0))
